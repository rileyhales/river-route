...
class UnitMuskingum(AbstractRouter):
    ...
    transformer_state_file  : path to a parquet file (n_basins, n_time_steps) to warm-start the state.
                     Only used when uh_kernel_file is also provided.

    Input data config keys (choose one source)
    -------------------------------------------
    # option 1 - pre-computed per-catchment runoff depth time series
    lateral_depth_files : list of 1 or more paths to netCDF files containing per-catchment runoff
                            depth time series for each river segment. The UH transformer converts
                            these depths to lagged per-segment lateral inflows before routing.
    # option 2 - gridded runoff depths converted via a weight table
    runoff_depth_grids    : list of 1 or more paths to netCDF files containing gridded runoff depth
                            time series. Requires grid_weights_file.
    grid_weights_file     : path to a netCDF weight table used to aggregate gridded depths to
                            per-catchment depths. Required when using runoff_depth_grids.
    ...
    def _validate_router_configs(self) -> None:
        # todo: normalize lateral_depth_files and runoff_depth_grids from str/Path to list,
        #  following the same pattern as TeleportMuskingum._validate_router_configs
        # todo: normalize runoff_depth_grids paths explicitly via os.path.abspath — the key lacks
        #  'file' so the base class path-normalization loop will not catch it
        # todo: raise ValueError if both lateral_depth_files and runoff_depth_grids are provided
        ...
        # todo: raise ValueError if runoff_depth_grids is set but grid_weights_file is not provided
        # todo: raise FileNotFoundError if grid_weights_file does not exist
        if (
            ...
            raise RuntimeError(
                ...
                'Set uh_type (and optionally uh_kernel_file/transformer_state_file) in config, '
    ...
    def _load_runoff(self) -> GeneratorSignature:
        ...
        #
        # option 1 — pre-computed per-catchment depth time series:
        #   iterate over zip(self.conf['lateral_depth_files'], self.conf['discharge_files']),
        #   open each netCDF, read dates and the depth array, yield
        #   (dates, depths_array, depth_file, discharge_file)
        #
        # option 2 — gridded depths aggregated via weight table:
        #   iterate over zip(self.conf['runoff_depth_grids'], self.conf['discharge_files']),
        #   call depth_to_volume(...) using self.conf['grid_weights_file'] and the grid file,
        #   yield (dates, depths_array, grid_file, discharge_file)
        #
        # note: the UH transformer in _router receives depths (not volumes) and is responsible
        #  for converting depth × area to a lagged lateral inflow rate.
    ...
    def route(self) -> Self:
        ...
        # todo write transformer final state if argument provided in config
    ...
    # todo why do we need separate set_vector functions? can we use the ones in the base class?
