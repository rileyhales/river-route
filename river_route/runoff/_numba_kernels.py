...
__all__ = ['aggregate_river', 'aggregate_to_rivers']
...
# No fastmath in this module, and aggregate_river must never be called from a fastmath kernel. fastmath lets the
# compiler assume values are never NaN, which removes the NaN replacement. Numba compiles a helper with its caller's
# fastmath flags and cache=True then serves that compilation to every later caller, so a single fastmath caller would
# silently remove the NaN replacement from aggregate_to_rivers too.
...
def aggregate_river(
    ...
    de-accumulation, clipping, NaN replacement, and the per-river scale. Each weight adds a contiguous cell series,
