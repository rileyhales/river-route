...
class Runoff:
    ...
    and aggregated onto rivers as area weighted depths or volumes in a single pass, written straight into a C-order
    (time, river) array that the routing kernels read without a copy.
    """
        ...
        return xr.Dataset(
            ...
            coords={
                'river_id': xr.DataArray(
                    self.river_ids.astype(np.int64, copy=False),
                    dims=('river_id',),
