"""
Tests for the premade discharge writers in river_route.router.writers, run on the synthetic network from conftest.
"""
...
def test_zarr_writer_splits_rivers_into_chunks(tmp_path):
    ...
    n_rivers = writers.ZARR_RIVERS_PER_CHUNK * 2 + 7
    router = rr.Router(
        rr.Configs(
            params_file=str(build_network(tmp_path / 'net', n_rivers=n_rivers).params_file),
...
def test_zarr_writer_packs_chunks_into_shards(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rr.router.writers, 'ZARR_CHUNKS_PER_SHARD', 2)
    n_rivers = writers.ZARR_RIVERS_PER_CHUNK * 3 + 7
    router = rr.Router(
        rr.Configs(
            params_file=str(build_network(tmp_path / 'net', n_rivers=n_rivers).params_file),
            discharge_files=[str(tmp_path / 'q.nc')],
...
def test_zarr_writer_threads_do_not_change_output(network: SyntheticNetwork):
    single = route_with_writer(network, 'q_single.zarr', writer=writers.zarr_writer)
    threaded = route_with_writer(network, 'q_threaded.zarr', writer=writers.zarr_writer, threads=4)
...
def test_parquet_writer_uses_the_configured_write_options(network: SyntheticNetwork, monkeypatch):
    monkeypatch.setattr(rr.router.writers, 'PARQUET_WRITE_OPTIONS', {'compression': 'zstd', 'use_dictionary': False})
    out = route_with_writer(network, 'q.parquet', writer=writers.parquet_writer)
...
def test_parquet_writer_replaces_an_existing_file(network: SyntheticNetwork):
    ...
    with open(out, 'w') as f:
        f.write('not parquet')
    route_with_writer(network, 'q.parquet', writer=writers.parquet_writer)
